...
export const utils = {
  onePointCoord(point: any, vm: any) {
  ...
  getPointerCoords(evt: any, vm: any) {
    let pointer;
  ...
  getPinchDistance(evt: any, vm: any) {
  ...
  getPinchCenterCoord(evt: any, vm: any) {
    let pointer1 = evt.touches[0];
    let pointer2 = evt.touches[1];
    let coord1 = this.onePointCoord(pointer1, vm);
    let coord2 = this.onePointCoord(pointer2, vm);
  ...
  rAFPolyfill() {
    ...
    for (
      ...
      window.cancelAnimationFrame =
        (window[(vendors[x] + 'CancelAnimationFrame') as any] as any) || // Webkit中此取消方法的名字变了
  ...
  getFileOrientation(arrayBuffer: any) {
  ...
  parseDataUrl(url: any) {
    const reg: any = /^data:([^;]+)?(;base64)?,(.*)/gim;
  ...
  // getRotatedImage(img: any, orientation: any) {
